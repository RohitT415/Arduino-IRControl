...
//#define speedPinR_2 9 // RIGHT PWM pin connect MODEL-X ENA, was 9
//#define RightDirectPin1_2 12 //Right Motor direction pin 1 to MODEL-X IN1, was 12
//#define RightDirectPin2_2 11 //Right Motor direction pin 2 to MODEL-X IN2, was 11
//#define speedPinL_2 6 // Left PWM pin connect MODEL-X ENB, was 6
//#define LeftDirectPin1_2 7 //Left Motor direction pin 1 to MODEL-X IN3, was 7
//#define LeftDirectPin2_2 8 //Left Motor direction pin 1 to MODEL-X IN4, was 8
...
//Left Motor direction pin 1 to MODEL-X IN3, was 7
 ...
 #define IR_NudgeRight     0x00FFB04F       //code from IR controller "#" button
 #define IR_NudgeLeft     0x00FF6897       //code from IR controller "*" button
...
void Nudge_Left_R(void)  //Turn left
{
  digitalWrite(RightDirectPin1_2, HIGH);
  digitalWrite(RightDirectPin2_2,LOW);
  digitalWrite(LeftDirectPin1_2,LOW);
  digitalWrite(LeftDirectPin2_2,HIGH);
  analogWrite(speedPinL_2,180);
  analogWrite(speedPinR_2,0);
  
}
void Nudge_Left(void)  //Turn left
{
  digitalWrite(RightDirectPin1, HIGH);
  digitalWrite(RightDirectPin2,LOW);
  digitalWrite(LeftDirectPin1,LOW);
  digitalWrite(LeftDirectPin2,HIGH);
  analogWrite(speedPinL,180);
  analogWrite(speedPinR,0);
  
}


void go_Right_R(void)  //Turn right
{
...
void Nudge_Right_R(void)  //Turn right
{
  digitalWrite(RightDirectPin1_2, LOW);  
  digitalWrite(RightDirectPin2_2,HIGH);
  digitalWrite(LeftDirectPin1_2,HIGH);  
...
void do_IR_Tick()
{
  if(IR.decode(&IRresults))
  {
    ...
    else if(IRresults.value==IR_NudgeRight)
    {
        Drive_Num=NUDGE_RIGHT; 
    }
    else if(IRresults.value==IR_NudgeLeft)
    {
        Drive_Num=NUDGE_LEFT; 
    }
    IRresults.value = 0;
    IR.resume();
  }
...
void do_Drive_Tick()
{
    switch (Drive_Num) 
    {
      ...
      case NUDGE_RIGHT: Nudge_Right();Nudge_Right_R();JogFlag = true;JogTimeCnt = 1;JogTime=millis();break;//if NUDGE_RIGHT code is detected, then turn right
      case NUDGE_LEFT: Nudge_Left();Nudge_Left_R();JogFlag = true;JogTimeCnt = 1;JogTime=millis();break;//if NUDGE_LEFT code is detected, then turn right
      default:break;
    }
    Drive_Num=DEF;
